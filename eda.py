...
- Pastikan file CSV memiliki kolom: 
    ['nama','stars','text','reviewernumberofreview','imageUrl','title','islocalguid',
     'reviewContext','reviewDetailRating','publishAt','publishAtDate']
    Jika ada perbedaan nama kolom, script akan mencoba memetakan nama umum.
- Ubah DATA_PATH di bawah
"""
...
# ---------------------------
DATA_PATH = "dataset_gabungan.csv"   # <-- Path ke file CSV yang sudah digabung
...
aliases = {
    ...
    'imageUrl': ['imageUrl', 'reviewImageUrls', 'reviewImageUrls/0'],
...
context_cols = [c for c in df.columns if _norm('reviewcontext') in _norm(c)]
...
# keep=False menandai semua kemunculan duplikat
...
print("Persentase review dengan foto: {:.2f}%".format(df['has_image'].mean()*100))
...
def compute_fake_score(row):
    ...
    except Exception: # Tetap aman jika ada error tak terduga
        pass
...
plt.hist(df['fake_score'], bins=df['fake_score'].max() + 1, align='left', rwidth=0.8)
...
sample_out = df[['nama','stars','text','fake_score','label','reason']].sample(50, random_state=42)
