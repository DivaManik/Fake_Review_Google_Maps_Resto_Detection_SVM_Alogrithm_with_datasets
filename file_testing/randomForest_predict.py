...
def predict_csv(input_csv, output_csv, model_path):
    ...
    # ========================================================
    # POST-PROCESSING RULE: Override jika gibberish terdeteksi
    # ========================================================
    override_count = 0
    override_flags = []

    # Check if gibberish columns exist
    has_gibberish_cols = ("tq_is_gibberish" in df.columns and "tq_gibberish_score" in df.columns)

    if has_gibberish_cols:
        print("\n[5] Applying post-processing rules...")
        for i in range(len(predicted_label)):
            is_gibberish = df["tq_is_gibberish"].iloc[i]
            gibberish_score = df["tq_gibberish_score"].iloc[i]

            # Rule: Jika text adalah gibberish (is_gibberish=1) dan
            # gibberish_score > 0.7, override ke FAKE
            if is_gibberish == 1 and gibberish_score > 0.7 and predicted_label[i] == "REAL":
                predicted_label[i] = "FAKE"
                override_count += 1
                override_flags.append(1)
                print(f"  Row {i+1}: REAL -> FAKE (gibberish_score={gibberish_score:.2f})")
            else:
                override_flags.append(0)

        print(f"  Total overridden: {override_count} predictions")
    else:
        override_flags = [0] * len(predicted_label)
        print("\n[5] Post-processing: gibberish columns not found, skipping...")

    df["predicted_label"] = predicted_label
    df["confidence"] = confidences
    df["decision_score"] = decision_scores
    df["gibberish_override"] = override_flags
    ...
    with open(summary_file, "w") as f:
        ...
        # Post-processing override stats
        f.write("\nPost-Processing (Gibberish Override):\n")
        f.write(f"  Total overridden REAL -> FAKE: {override_count}\n")
        if override_count > 0:
            override_indices = [i+1 for i, flag in enumerate(override_flags) if flag == 1]
            f.write(f"  Overridden rows: {override_indices}\n")
